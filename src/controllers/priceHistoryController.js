...
exports.getAllPriceHistories = async (req, res) => {
  try {
    ...
    const data =
      await priceHistoryService.getAllPriceHistories(inventory_item_id);
...
exports.createPriceHistory = async (req, res) => {
  try {
    ...
    if (!inventory_item_id || new_price == null) {
      return res.status(400).json({
        message: 'Faltan datos: inventory_item_id y new_price son requeridos',
      });
    }

    // Llamada al service: se encargará de obtener old_price automáticamente
    const data = await priceHistoryService.createPriceHistory(
      inventory_item_id,
      new_price
    );
