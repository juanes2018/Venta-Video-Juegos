...
exports.createStockReservation = async (req, res) => {
    ...
    if (!inventory_item_id || !durationMinutes == null) {
...
exports.getAllStockReservations = async (req, res) => {
    try{
        const reservations = await stockReservationService.getAllStockReservations();
...
exports.getStockReservationById = async(req, res) => {
    try{
        const reservation = await stockReservationService.getStockReservationById(req.params.id);
...
exports.deleteStockReservation = async (req, res) => {
    try{
        const deleted = await stockReservationService.deleteStockReservation(req.params.id);
        
        if(!deleted) return res.status(404).json({message: 'Reserva no encontrada' });
        res.json({ message: 'Reserva eliminada'})

    }catch (error){
        res.status(500).json({ message: 'Error eliminando reserva', error: error.message });
