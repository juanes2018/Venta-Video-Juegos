...
exports.getAllProductVariants = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al obtener variantes de producto', error });
    }
};
...
exports.getProductVariantById = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al obtener variante de producto', error });
    }
};
...
exports.updateProductVariant = async (req, res) => {
    try {
        const { id } = req.params;
        const updatedVariant = await productVariantService.updateProductVariant(id, req.body);
        res.status(200).json({
            message: "Variante de producto actualizada",
            data: updatedVariant
    });
    } catch (error) {
        res.status(500).json({ message: 'Error al actualizar variante de producto', error });
    }
};

exports.deleteProductVariant = async (req, res) => {
    try {
        const deletedProductVariant = await productVariantService.deleteProductVariant(req.params.id);
        ...
        res.status(500).json({ message: 'Error al eliminar variante de producto', error });
    }
};     