...
function generateSlug(title) {
   ...
   exports.createProduct = async (productData) => {
    const { title, description, franchise, developer, publisher, release_year, price, stock, series } = productData;
...
exports.updateProduct = async (id, productData) => {
    ...
    const slug = title ? title.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') : undefined;
    ...
    if (franchise) { fields.push('franchise = ?'); values.push(franchise); }
    if (developer) { fields.push('developer = ?'); values.push(developer); }
    ...
    if (series) { fields.push('series = ?'); values.push(series); }
